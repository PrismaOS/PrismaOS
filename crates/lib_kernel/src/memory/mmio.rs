...
use x86_64::{
    structures::paging::{
        Page, PageTable, PageTableFlags, PhysFrame, Mapper,
...
/// Production-ready MMIO mapper that integrates with kernel memory management
...
impl MmioMapper {
    ...
    pub unsafe fn map_mmio(&mut self, phys_addr: PhysAddr, size: usize) -> Result<VirtAddr, MapToError<Size4KiB>> {
        ...
        // Use high memory area for MMIO mappings (0xFFFF_8000_0000_0000+)
        ...
        for i in 0..page_count {
            ...
            let page_virt = Page::from_start_address(
        ...
        for i in 0..page_count {
            let page_virt = Page::from_start_address(virt_addr + (i as u64 * 0x1000))
                .map_err(|_| MapToError::FrameAllocationFailed)?;
            self.mapper.flush(page_virt);
        }
    ...
    pub fn unmap_mmio(&mut self, virt_addr: VirtAddr, size: usize) -> Result<(), MapToError<Size4KiB>> {
        ...
        if let Some(region) = regions.remove(&(virt_addr.as_u64() as usize)) {
            ...
            for i in 0..region.page_count {
                let page = Page::from_start_address(aligned_virt + (i as u64 * 0x1000))
                    .map_err(|_| MapToError::FrameAllocationFailed)?;
    ...
    fn is_virtual_region_free(&self, addr: VirtAddr, size: usize) -> bool {
        ...
        for i in 0..page_count {
            let page = Page::from_start_address(addr + (i as u64 * 0x1000));
            if let Ok(page) = page {
...
impl XhciMmioMapper {
    ...
    /// Create a new XhciMmioMapper with explicit memory management (for advanced use)
    pub fn new_with_memory_management(
...
pub fn init_mmio_mapping() {
    crate::kprintln!("Initializing MMIO mapping system");
    // MMIO regions tracker is already initialized via lazy_static
}
