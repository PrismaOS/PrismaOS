...
use x86_64::{
    structures::paging::{
        Page, PageTableFlags, PhysFrame, Mapper, OffsetPageTable, MappedPageTable,
...
/// Unit type for default PageTableFrameMapping implementation
unsafe impl PageTableFrameMapping for () {
    fn frame_to_pointer(&self, _frame: PhysFrame) -> *mut x86_64::structures::paging::PageTable {
        core::ptr::null_mut()
    }
}

/// Production-ready MMIO mapper with concrete types
pub enum MmioMapper<P: PageTableFrameMapping + 'static = ()> {
    OffsetPageTable(OffsetPageTableMapper),
    MappedPageTable(MappedPageTableMapper<P>),
}

/// MMIO mapper using OffsetPageTable
pub struct OffsetPageTableMapper {
    mapper: &'static mut OffsetPageTable<'static>,
    frame_allocator: &'static mut dyn FrameAllocator<Size4KiB>,
}

/// MMIO mapper using MappedPageTable with proper generic parameter
pub struct MappedPageTableMapper<P: PageTableFrameMapping + 'static> {
    mapper: &'static mut MappedPageTable<'static, P>,
    frame_allocator: &'static mut dyn FrameAllocator<Size4KiB>,
}

impl<P: PageTableFrameMapping> MmioMapper<P> {
    /// Create a new MMIO mapper with OffsetPageTable
    pub fn new_offset_page_table(
        mapper: &'static mut OffsetPageTable<'static>,
        frame_allocator: &'static mut dyn FrameAllocator<Size4KiB>,
    ) -> Self {
        Self::OffsetPageTable(OffsetPageTableMapper {
            mapper,
            frame_allocator,
        })
    }

    /// Create a new MMIO mapper with MappedPageTable
    pub fn new_mapped_page_table(
        mapper: &'static mut MappedPageTable<'static, P>,
        frame_allocator: &'static mut dyn FrameAllocator<Size4KiB>,
    ) -> Self {
        Self::MappedPageTable(MappedPageTableMapper {
            mapper,
            frame_allocator,
        })
    }
    ...
    pub unsafe fn map_mmio(&mut self, phys_addr: PhysAddr, size: usize) -> Result<VirtAddr, MapToError<Size4KiB>> {
        match self {
            MmioMapper::OffsetPageTable(mapper) => mapper.map_mmio(phys_addr, size),
            MmioMapper::MappedPageTable(mapper) => mapper.map_mmio(phys_addr, size),
        }
    }

    /// Unmap a previously mapped MMIO region
    pub fn unmap_mmio(&mut self, virt_addr: VirtAddr, size: usize) -> Result<(), &'static str> {
        match self {
            MmioMapper::OffsetPageTable(mapper) => mapper.unmap_mmio(virt_addr, size),
            MmioMapper::MappedPageTable(mapper) => mapper.unmap_mmio(virt_addr, size),
        }
    }
}

impl OffsetPageTableMapper {
    /// Map a physical MMIO region to virtual memory with appropriate flags
    pub unsafe fn map_mmio(&mut self, phys_addr: PhysAddr, size: usize) -> Result<VirtAddr, MapToError<Size4KiB>> {
...
impl<P: PageTableFrameMapping> MappedPageTableMapper<P> {
    /// Map a physical MMIO region to virtual memory with appropriate flags
    pub unsafe fn map_mmio(&mut self, phys_addr: PhysAddr, size: usize) -> Result<VirtAddr, MapToError<Size4KiB>> {
        // Ensure proper alignment
        let aligned_phys = PhysAddr::new(phys_addr.as_u64() & !0xFFF);
...
pub enum XhciMmioMapper {
    OffsetPageTable(MmioMapper<()>),
    MappedPageTable(Box<dyn XhciMmioMapperTrait + Send + Sync>),
}

/// Trait for MappedPageTable wrapper to handle different PageTableFrameMapping types
pub trait XhciMmioMapperTrait {
    unsafe fn map(&mut self, phys_addr: PhysAddr, size: usize) -> Result<VirtAddr, MapToError<Size4KiB>>;
    fn unmap(&mut self, virt_addr: VirtAddr, size: usize) -> Result<(), &'static str>;
}

impl<P: PageTableFrameMapping + Send + Sync + 'static> XhciMmioMapperTrait for MmioMapper<P> {
    unsafe fn map(&mut self, phys_addr: PhysAddr, size: usize) -> Result<VirtAddr, MapToError<Size4KiB>> {
        self.map_mmio(phys_addr, size)
    }

    fn unmap(&mut self, virt_addr: VirtAddr, size: usize) -> Result<(), &'static str> {
        self.unmap_mmio(virt_addr, size)
    }
}
...
impl XhciMmioMapper {
    ...
    pub fn new_offset_page_table(
        ...
        Self::OffsetPageTable(MmioMapper::new_offset_page_table(mapper, frame_allocator))
    }

    /// Create a new XhciMmioMapper using MappedPageTable
    pub fn new_mapped_page_table<P: PageTableFrameMapping + Send + Sync + 'static>(
        mapper: &'static mut MappedPageTable<'static, P>,
        frame_allocator: &'static mut dyn FrameAllocator<Size4KiB>,
    ) -> Self {
        let mmio_mapper = MmioMapper::new_mapped_page_table(mapper, frame_allocator);
        Self::MappedPageTable(Box::new(mmio_mapper))
    }

    /// Create a new XhciMmioMapper using the kernel's memory management
    pub fn new() -> Result<Self, &'static str> {
        let mapper = super::get_kernel_mapper()
            .ok_or("Kernel mapper not available")?;
        let frame_allocator = super::get_kernel_frame_allocator()
            .ok_or("Kernel frame allocator not available")?;

        // We need to determine which type of mapper we have
        // For now, we'll assume OffsetPageTable since it's more common
        // In production, this would be determined by the kernel's memory management setup
...
impl xhci::accessor::Mapper for XhciMmioMapper {
    unsafe fn map(&mut self, phys_addr: usize, bytes: usize) -> core::num::NonZeroUsize {
        ...
        let result = match self {
            XhciMmioMapper::OffsetPageTable(mapper) => mapper.map_mmio(phys, bytes),
            XhciMmioMapper::MappedPageTable(mapper) => mapper.map(phys, bytes),
        };
    ...
    fn unmap(&mut self, virt_addr: usize, bytes: usize) {
        ...
        let result = match self {
            XhciMmioMapper::OffsetPageTable(mapper) => mapper.unmap_mmio(virt, bytes),
            XhciMmioMapper::MappedPageTable(mapper) => mapper.unmap(virt, bytes),
        };
